...
class TestDDPG(TestCase):
    ...
    def _test_tic_tac_toe_mlp_with_ntm(self) -> None:
        """
        ...
        https://en.wikipedia.org/wiki/Tic-tac-toe
        Цель эксперимента заключается в провеке способности ntm-контроллера обучится эффективному использованию
        внешнего банка памяти при использовании Deep Q-learning метода.

        NTM используется для хранения текущего статуса игры и хранения другой информации,
        какая потребуется NTM-котроллеру.
        Структура памяти: [key game state, current game state, ...]
        Здесь key game state константный ключ состояния игры.
        Агент может несколько раз, но не превышая некий фиксированный верхний предел, откладывать ход,
        манипулируя при этом содержимым банка памяти.
        Агент может менять все блоки памяти, вкючая первые два, отведенные под key game state и current game state.
        Допустимые сдвиги для процедуры фокусирования (-1, 0, 1).
